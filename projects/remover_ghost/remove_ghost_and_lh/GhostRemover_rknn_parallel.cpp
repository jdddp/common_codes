    ...
    float mirror_ratio_thre)
{
    ...
    for (const auto& box_info : cy_boxes) {
        ...
        MirrorFillRequest request;
        request.main_bbox = main_bbox;
        request.merged_bbox = merged_bbox;
        request.influence_rect =
            buildMirrorInfluenceRect(merged_bbox, result_img.size());
        fill_requests.push_back(request);
    }
    ...
    const std::vector<Object_LH>& objects)
{
    ...
    for (const auto& obj_info : objects) {
        ...
        LhBoxInfo lh_box_info;
        lh_box_info.box = detected_box;
        lh_box_info.label_id = obj_info.label_id;
        lh_box_info.label = obj_info.label;
        lh_box_info.prob = obj_info.prob;
        last_lh_boxes_.push_back(lh_box_info);
    ...
    const cv::Rect& image_rect)
{
    ...
    for (const auto& obj : objects) {
        ...
        CurrentCyDetection detection_info;
        detection_info.box = box;
        detection_info.allow_prediction = isIsolatedCyCandidate(box, objects);
        current_detections.push_back(detection_info);
    }
