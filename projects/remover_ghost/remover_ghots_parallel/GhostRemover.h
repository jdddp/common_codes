...
private:
    ...
    std::vector<cv::Rect> last_yqfish2_boxes_;
...
public:
    ...
    const std::vector<cv::Rect>& getLastYqBoxes() const;
...
private:
    ...
    // 对不需要追踪的类别，直接根据当前帧检测结果返回修补框。
    std::vector<cv::Rect> collectImmediateBoxesByLabel(
        const std::vector<Object>& objects,
