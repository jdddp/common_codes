...
cv::Mat GhostRemover::removeGhosts(const cv::Mat& input_img,
    ...
    for (const auto& box_info : cy_boxes) {
        ...
        result_img = mirrorFill(result_img, main_bbox, merged_bbox);
    }
...
//            // 只有當參考區長度足夠覆蓋整個待填充寬度時，才視為可用。
//            const bool hasL = lenL >= fill_len;
//            const bool hasR = lenR >= fill_len;
//            if (!hasL && !hasR) {
//                // 左右参考都不足以覆盖待填充区时，该行无法安全补值。
//                continue;
//            }
//
    ...
    for (auto& plane : planes) {
        ...
        if (mirror_ratio_thre > 0.f && hasL && hasR) {
            ...
            cv::minMaxLoc(plane(cv::Range(y1, y2), cv::Range(r_x1, r_x2)),
                nullptr,
                &right_max);
            std::cout << "left_max: " << left_max << " right_max: " << right_max << std::endl;
            float max_v = std::max<float>(left_max, right_max);
            float min_v = std::min<float>(left_max, right_max);
            float actual_ratio = max_v / min_v;
            std::cout << "ratio: " << actual_ratio << std::endl;
