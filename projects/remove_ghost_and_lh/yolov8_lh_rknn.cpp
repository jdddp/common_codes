...
float YoloV8RKNN_LH::DetectBox::area() const
{
    return std::max(0.f, xmax - xmin) * std::max(0.f, ymax - ymin);
}

float YoloV8RKNN_LH::DetectBox::iou(const DetectBox& other) const
{
...
bool YoloV8RKNN_LH::load(const std::string& model_path)
{
    if (ctx_ != 0) {
        rknn_destroy(ctx_);
        ctx_ = 0;
    ...
    const int core_mask_ret = rknn_set_core_mask(ctx_, RKNN_NPU_CORE_1);
    std::cout << "[YoloV8RKNN_LH::load] rknn_set_core_mask(core1) ret="
...
bool YoloV8RKNN_LH::query_io_attr()
{
...
void YoloV8RKNN_LH::preprocess(const cv::Mat& bgr, cv::Mat& rgb_resized) const
{
    cv::Mat resized;
    cv::resize(bgr,
        resized,
...
std::vector<Object_LH> YoloV8RKNN_LH::detect(const cv::Mat& img)
{
    std::vector<Object_LH> objects;
...
void YoloV8RKNN_LH::postprocess(const std::vector<rknn_output>& outputs,
    int img_h,
    int img_w,
    std::vector<Object_LH>& objects) const
{
...
void YoloV8RKNN_LH::decode_head(const float* cls_data,
...
float YoloV8RKNN_LH::sigmoid(float x) const
{
    return 1.f / (1.f + std::exp(-x));
}

float YoloV8RKNN_LH::softmax_normalize(const float* data,
    int index,
    int h,
    int w,
    int map_h,
...
void YoloV8RKNN_LH::nms(std::vector<DetectBox>& detect_result,
...
void YoloV8RKNN_LH::stage2_filter(std::vector<DetectBox>& detect_result) const
{
